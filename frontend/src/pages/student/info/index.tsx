import { changeAvatar } from '@/service/common';
...
const StudentInfo = () => {
  ...
  const avatarSuccess = (info: any) => {
    console.log(info);
    if (info.file.status === 'done') {
      changeAvatar({ avatar: info.fileList[0]?.response.data.path }).then(
        (res) => {
          message.success('上传成功');
          refresh();
          setUrl(info.fileList[0]?.response.data.path);
        },
      );
    }
  };
  return (
    <div className={styles.wrapper}>
      <Button
        type="primary"
        style={{ marginRight: 20, position: 'absolute', left: 0 }}
      ...
      >
        ...
        <Item name="name" label="性别">
          <Radio.Group>
            <Radio>男</Radio>
            <Radio>女</Radio>
          </Radio.Group>
        </Item>
        <Item name="name" label="班级">
          {type ? 'xxx' : <Input style={{ width: 300 }} />}
        </Item>
        <Item name="name" label="学院">
          {type ? 'xxxxs' : <Input style={{ width: 300 }} />}
        </Item>
        <Item name="name" label="系别">
          {type ? 'xxxxs' : <Input style={{ width: 300 }} />}
        </Item>
        <Item name="name" label="学号">
          {type ? 'xxxxs' : <Input style={{ width: 300 }} />}
        </Item>
        <Item name="name" label="手机号">
          {type ? 'xxxxs' : <Input style={{ width: 300 }} />}
        </Item>
        <Item name="name" label="密码">
          *****
          {pass ? (
            <EyeTwoTone
              style={{ marginLeft: 20 }}
